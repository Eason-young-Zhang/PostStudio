...
适用版本：**0.4.6 本地测试版** · 文档更新日期：2026 年 9 月 22 日

> 本手册按 0.4 的实际界面编写，并附离线 HTML 版本。验证范围和试用边界见[验收记录](ACCEPTANCE_0_4.md)。

PostStudio 是一个在 Mac 上离线使用的图像创作工作台。你可以把照片自由摆放在画布上，使用拼图、分块取样、反转色、配色提取、边框和水印工具生成独立的新稿，再比较、继续加工或导出作品。素材与处理留在本机，不需要账户或网络连接。
...
边框、水印、渐变、阴影和配色精细布局的数值参数均采用统一两行布局：上方左侧为参数名称、右侧为紧凑精确输入框，下方滑动条横跨整个参数行；分块取样保留原有滑条。拖动滑条可直观调整，点击输入框可输入准确数值，按 Enter 或离开输入框后生效。

滑条取得焦点后，方向键按该参数步长微调，Page Up / Down 以更大步幅调整。常用小数步长为 0.1；配色间距与留边也支持小数滑动。选择项、文字、字体和颜色仍使用各自控件。

范围较大的参数先提供常用滑动范围；在输入框输入范围外的合法值后，滑条自动扩展以包含该值。切换像素／百分比单位会调整常用滑动范围，参数数值含义沿用原规则，不自动换算。预设及项目中的精确数值仍保留。
...
右侧“提色”和“排版”分别显示各自内容，切换后按当前页所需高度显示；提色操作按钮紧随色卡列表。拖动列表底部的蓝色短横条，可上下调整列表高度；双击恢复默认。调节条获得焦点后，方向键每次调整 10 像素，Shift＋方向键调整 40 像素。高度在当前会话的页签切换中保留，不写入图像参数。
...
![拼图工具：统一内框与外框](images/collage.png)

### 20.1 从选图到一张作品

1. 在画布或总览中选中 1–10 张图像，点击右侧 **拼图**。本工具把所选图片合成一张作品；不会分别对每张图生成一个结果。
2. 在 **布局** 页选择网格、竖向排列、横向排列或主图＋辅图，点击 **使用此布局**。更换布局保留图片及其显示方式，会重置分隔线位置。
3. 选择 3:4、4:5、1:1、4:3，或选自定义后分别调整输出宽度与高度。宽高包含外框；固定比例时修改任一边会联动另一边。所有尺寸均可滑动、精确输入与方向键微调。
4. 在预览中拖动一张图片到另一张上，交换位置；鼠标靠近分隔线后拖动，可改变相邻区域的大小。改变区域不会改变原图像素文件。
5. 点击 **生成拼图 · 返回画布**，会生成独立的 16 位中间产物。随后可加水印、继续使用其他工具，或通过工作台的导出功能输出 JPEG、PNG、TIFF。

比例是创作预设，并非社媒平台尺寸上限。首版输出上限为 5000 万像素，单边最多 20000px；导出时仍可降采样。拖动预览使用缓存缩略图，正式产物由项目中的工作原图重新绘制。

### 20.2 完整显示与裁切

默认 **完整显示（不裁切）**，保留照片全貌；照片与区域比例不同时会出现底色留白。要让区域铺满，请先点选图片，再到 **图片** 页选择 **裁切填满**。

水平／垂直位置滑条决定照片在区域内的位置。也可将拖动模式切为 **调整图片位置**，直接拖动照片；切回 **交换图片** 后拖动恢复换位。完整显示模式移动的是留白中的位置，裁切模式移动的是裁切视野。未被裁切或没有留白的方向不会发生位移。

### 20.3 内框与外框

**统一内框** 是整张拼图沿分隔线形成的框线，不是每张图片各画一圈边。可调整分隔宽度、颜色与不透明度；交叉处不会重复叠加厚度或透明度。框线占用图片区域之间的空间，不另设重复的间距参数。

**外框与留白** 可分别调整上、右、下、左四边，也可以选择四边同步。底色同时用于外框和完整显示模式下的留白。内框与外框独立，不会因修改一项而改变另一项。框宽过大、导致图片区域不足时，页面显示提示并禁止生成；减小框宽即可恢复。

### 20.4 修改、撤销与分享

编辑页提供撤销／重做按钮和 Command+Z／Command+Shift+Z。返回画布不会自动生成作品；需要保留结果时先点击生成。

在工作台选中拼图，点击 **重新编辑**，可恢复来源图片、布局、裁切位置与框线。无下游引用时可 **保存修改**，也可生成独立新版本；有后续处理或组合引用时，只能生成新版本。修改原图的其他版本不会自动改变已有拼图。

保存项目后，复制一个 `.blockproj` 文件即可带走全部来源与拼图参数；含拼图的项目需要 0.4.6 或更新版本打开。首版拼图不进入逐张执行的预设／工作流；自动将一组照片分配成多张社媒拼图留待后续版本。
