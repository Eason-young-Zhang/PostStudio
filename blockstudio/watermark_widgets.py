...
class WatermarkControls(QWidget):
    ...
    def __init__(self):
        ...
        f=QFormLayout();f.setRowWrapPolicy(QFormLayout.RowWrapPolicy.WrapLongRows);v.addLayout(f);self.font=QFontComboBox();self.font.currentFontChanged.connect(self.font_changed);f.addRow('字体',self.font);self.font_note=QLabel();self.font_note.setWordWrap(True);f.addRow(self.font_note)
        ...
        for key,label,lo,hi in [('size','字号',.1,1000),('width','水印宽度 %',.1,200),('x','水平偏移',-10000,10000),('y','垂直偏移',-10000,10000),('opacity','不透明度 %',0,100),('rotation','旋转 °',-360,360),('spacing','字距（字号 %）',-50,200),('line_height','行距 %',50,300),('stroke','描边（字号 %）',0,100)]:
            n=number(0,lo,hi);self.values[key]=n;f.addRow(label,n);n.valueChanged.connect(self.change)
        self.color_buttons={}
        ...
        hint=QLabel('在预览中拖动当前水印；箭头微移，Shift 加大步幅。字号单位相对定位区域短边。');hint.setWordWrap(True);v.addWidget(hint);self.refresh()
    def refresh(self):
    ...
    def select(self,i):
        ...
        self.shadow.load(p['shadow']);self.loading=False
