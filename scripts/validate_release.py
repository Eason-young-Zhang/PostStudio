...
def wait_until(predicate,timeout=30):
    ...
    while not predicate():
        if time.monotonic()>end:raise RuntimeError('UI validation timeout')
...
with tempfile.TemporaryDirectory(prefix='blockstudio-validation-') as temp:
    ...
    while time.monotonic()-start<10:app.processEvents();QTest.qWait(2)
...
Path('artifacts/release-validation.json').write_text(json.dumps(report,indent=2,ensure_ascii=False));print(json.dumps(report,indent=2,ensure_ascii=False))
