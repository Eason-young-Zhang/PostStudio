...
**0.4.1 本地测试版**，主要在 Apple Silicon Mac 上开发与验证。素材和图像处理留在本机；安装依赖后可离线运行。尚未提供经过 Apple 公证的安装包。
...
当前 62 项测试通过。构建输出为 `dist/PostStudio.app`；构建脚本需要 macOS Command Line Tools，本机签名和公证不由此脚本提供。
